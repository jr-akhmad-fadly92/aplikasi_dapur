...
class DetailPengajuanMenuController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('exports.detail_pengajuan_menu_1');
    }

    public function exportExcel(Request $request, $menuId)
    {
        $data_dapur = Datadapur::first();
        $data_menu = DB::table('tb_menu')->where('id', $menuId)->first();
        
        $jumlah_pax = DB::table('rincian_sekolah')->where('id_menu_harian', $menuId)->sum('jumlah_penerima_total');
        $jumlah_pax_a = DB::table('rincian_sekolah')->where('id_menu_harian', $menuId)->sum('jumlah_penerima_a');
        $jumlah_pax_b = DB::table('rincian_sekolah')->where('id_menu_harian', $menuId)->sum('jumlah_penerima_b');
        $keterangan_pax = 'ANAK SEKOLAH PAX A';
        if($jumlah_pax_b > 0) {
            $keterangan_pax = 'ANAK SEKOLAH PAX B';
        }
        ...
        $rows_rekap_bumbu =  DB::table('tb_menu as m')
            ...
            'menuId' => $menuId,
            'keterangan_pax' => $keterangan_pax,
            'jumlah_pax_a' => $jumlah_pax_a,
            'jumlah_pax_b' => $jumlah_pax_b,
            'jumlah_pax' => $jumlah_pax,
        ];

        return \Maatwebsite\Excel\Facades\Excel::download(
            new \App\Exports\DetailPengajuanMenuExport($data),
            'detail_pengajuan_menu.xlsx'
        );
    }
}
