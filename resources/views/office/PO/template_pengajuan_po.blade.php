    ...
    <div class="container">
        ...
        <table style="margin-top: 20px;">
            ...
            <tr>
                ...
                <td>{{ $row->bahan }}</td>
                <td style="text-align: right;">{{ rtrim(rtrim(number_format($row->jumlah_bahan, 3, ',', '.'), '0'), ',') }}</td>
                <td style="text-align: right;">{{ $row->satuan }}</td>
                <td style="text-align: right;">{{ number_format($row->jumlah_bahan > 0 ? ($row->jumlah_po / $row->jumlah_bahan) : 0, 0, ',', '.') }}</td>
                <td style="text-align: right;">{{ number_format($row->jumlah_po, 0, ',', '.') }}</td>
            </tr>
            @php $total_semua += $row->jumlah_po; @endphp
            @endforeach
            <tr>
        ...
        <!-- Footer Tanda Tangan -->
        <div class="footer">
            <table style="font-size: 13px; margin-top: 30px;" class="no-border">
                <tr>
                    <td style="text-align: center; width: 50%;">Ahli Akuntan</td>
                    <td style="text-align: center; width: 50%;">Kepala Dapur</td>
                </tr>
                <tr>
